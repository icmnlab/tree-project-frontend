...
> **版本紀錄說明（交接用）**
> - 本檔依**版本號＋日期**倒序記錄里程碑；**請保留**——刪除不符合一般交接慣例。
> - **2026-03 ～ 2026-06-10** 部分小版（如 v18.3.x～v18.4.x 中間）未逐條寫入；細節見 git history 或 `docs/WORK_STATUS.md`。2026-06-11 起已恢復逐版記錄。
> - 版本以 `pubspec.yaml` 的 `version:` 為準。

---

## v18.10.4 (2026-06-15) — 定位並修正其餘兩處 RenderFlex 溢位
...
- 備註：log 另見「11px 底／78px 右」兩處溢位已於 **v18.10.4** 定位修正。
...
- **配合後端修正**：選定樹況後由 `update_v2` 連動 `lifecycle_status`（枯立木→dead、倒塌→fallen…），不再出現「編輯標記枯死卻仍計為活立木」。
- 備註：此頁為相容舊流程之靜態清單；V3 新增頁已為完整動態目錄（`TreeStatusService`），後續可統一改接。
