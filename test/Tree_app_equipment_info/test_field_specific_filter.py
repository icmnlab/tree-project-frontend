#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
測試 Field-Specific 過濾器 (v13.2 完整版)
針對每個欄位套用特定的驗證規則
"""

import re

def extract_from_android_log(log_file):
    """從 Android BLE Log 重建（v13.1 過濾器）"""
    with open(log_file, 'r', encoding='utf-16') as f:
        content = f.read()
    
    raw_fragments = []
    for line in content.splitlines():
        if "[BLE RAW]" in line:
            parts = line.split("[BLE RAW]")
            if len(parts) > 1:
                hex_str = parts[1].strip()
                if hex_str:
                    raw_fragments.append(hex_str)
    
    # 重組 byte stream
    full_byte_stream = []
    for hex_line in raw_fragments:
        clean_hex = hex_line.replace(' ', '')
        for i in range(0, len(clean_hex), 2):
            if i+2 <= len(clean_hex):
                try:
                    full_byte_stream.append(int(clean_hex[i:i+2], 16))
                except:
                    pass
    
    # v13.1 Byte-Level 過濾器
    cleaned_data = []
    i = 0
    
    while i < len(full_byte_stream):
        is_header = False
        if i + 2 < len(full_byte_stream):
            if (full_byte_stream[i] == 0x44 and full_byte_stream[i+1] == 0xCD and full_byte_stream[i+2] == 0x00) or \
               (full_byte_stream[i] == 0x44 and full_byte_stream[i+1] == 0x36 and full_byte_stream[i+2] == 0x00):
                is_header = True
                
                if len(cleaned_data) >= 2 and (cleaned_data[-1] > 0x7E or cleaned_data[-2] > 0x7E):
                    cleaned_data.pop()
                    cleaned_data.pop()
                elif len(cleaned_data) == 1 and cleaned_data[-1] > 0x7E:
                    cleaned_data.pop()
                
                i += 3
                continue
        
        if full_byte_stream[i] > 0x7E and full_byte_stream[i] not in [0x0D, 0x0A]:
            i += 1
            continue
        
        cleaned_data.append(full_byte_stream[i])
        i += 1
    
    # 解碼
    try:
        decoded_text = bytes(cleaned_data).decode('utf-8', errors='ignore')
    except:
        decoded_text = bytes(cleaned_data).decode('latin-1', errors='ignore')
    
    # String-Level 白名單
    cleaned_text = re.sub(r'[^0-9A-Z\.\;\-\r\n\$\#]', '', decoded_text)
    
    # 提取數據行
    data_lines = []
    for line in cleaned_text.split('\n'):
        line = line.strip()
        if line.startswith('$') and len(line) > 10:
            data_lines.append(line)
    
    return data_lines

def apply_field_specific_filter(data_lines):
    """
    套用 Field-Specific 過濾器
    每個欄位有自己的驗證規則
    """
    # VLGEO CSV 欄位索引定義
    # 參考: https://www.haglofsweden.com/
    FIELD_RULES = {
        2: ('TYPE', r'^(1P|3P|3D|DME|)$'),  # 只允許特定類型
        13: ('N/S', r'^[NS]?$'),  # 只允許 N 或 S
        15: ('E/W', r'^[EW]?$'),  # 只允許 E 或 W
        32: ('UTM', r'^[0-9]{1,2}[A-Z]?$'),  # UTM Zone: 51Q, 51R
        # 數字欄位：只允許數字、小數點、負號
        12: ('LAT', r'^-?[0-9]*\.?[0-9]*$'),
        14: ('LON', r'^-?[0-9]*\.?[0-9]*$'),
        23: ('SD', r'^-?[0-9]*\.?[0-9]*$'),
        24: ('HD', r'^-?[0-9]*\.?[0-9]*$'),
        25: ('H', r'^-?[0-9]*\.?[0-9]*$'),
        27: ('PITCH', r'^-?[0-9]*\.?[0-9]*$'),
        28: ('AZ', r'^-?[0-9]*\.?[0-9]*$'),
    }
    
    cleaned_lines = []
    
    for line in data_lines:
        fields = line.split(';')
        
        # 清理每個欄位
        for idx in range(len(fields)):
            # Context-Aware: 移除數字欄位中的大寫字母
            # (TYPE, N/S, E/W, UTM 除外)
            if idx not in [2, 13, 15, 32]:
                fields[idx] = re.sub(r'[A-Z]', '', fields[idx])
            
            # Field-Specific: TYPE 欄位特殊處理
            if idx == 2:
                # 如果 TYPE 不是合法值，嘗試修正
                type_val = fields[idx]
                if type_val and type_val not in ['1P', '3P', '3D', 'DME', '']:
                    # 移除雜訊字母，保留可能的合法部分
                    # 例如：'1RP' → 檢查是否包含 '1P'
                    for valid_type in ['1P', '3P', '3D', 'DME']:
                        if valid_type in type_val:
                            fields[idx] = valid_type
                            break
                    else:
                        # 無法修正，移除所有非合法字元
                        fields[idx] = re.sub(r'[^13DP]', '', type_val)[:2]
        
        cleaned_lines.append(';'.join(fields))
    
    return cleaned_lines

def test_field_specific_filter():
    """測試 Field-Specific 過濾器"""
    print("=" * 80)
    print(" 測試 Field-Specific 過濾器 (v13.2 完整版)")
    print("=" * 80)
    print()
    
    # 1. 從我們的 Log 重建
    print("[Step 1] 從 ble_debug_log.txt 重建...")
    our_lines = extract_from_android_log('tree_project/project_code/frontend/ble_debug_log.txt')
    print(f"  v13.1 重建: {len(our_lines)} 筆")
    print()
    
    # 2. 套用 Field-Specific 過濾
    print("[Step 2] 套用 Field-Specific 過濾器...")
    print("  規則 1: 數字欄位移除所有 A-Z")
    print("  規則 2: TYPE 欄位只接受 1P/3P/3D/DME")
    print()
    
    enhanced_lines = apply_field_specific_filter(our_lines)
    print(f"  增強後: {len(enhanced_lines)} 筆")
    print()
    
    # 3. Last Record Wins
    our_by_id = {}
    for line in enhanced_lines:
        fields = line.split(';')
        if len(fields) > 6:
            id_clean = re.sub(r'[^0-9]', '', fields[6])
            if id_clean:
                our_by_id[id_clean] = line
    
    # 4. 讀取官方輸出
    print("[Step 3] 讀取官方 Android App 輸出...")
    with open('tree_project/Tree_app_equipment_info/DATA_2.CSV', 'r', encoding='utf-8') as f:
        official_lines = [l.strip() for l in f if l.strip().startswith('$')]
    
    official_by_id = {}
    for line in official_lines:
        fields = line.split(';')
        if len(fields) > 6:
            id_clean = re.sub(r'[^0-9]', '', fields[6])
            if id_clean:
                official_by_id[id_clean] = line
    
    print(f"  官方 Android: {len(official_by_id)} 個唯一 ID")
    print()
    
    # 5. 比對
    print("[Step 4] 比對結果...")
    print("=" * 80)
    
    matches = 0
    differences = []
    
    for id_val in sorted(official_by_id.keys(), key=lambda x: int(x) if x.isdigit() else 0):
        if id_val in our_by_id:
            if our_by_id[id_val] == official_by_id[id_val]:
                matches += 1
            else:
                differences.append({
                    'id': id_val,
                    'ours': our_by_id[id_val],
                    'official': official_by_id[id_val]
                })
    
    total = len(official_by_id)
    accuracy = matches / total * 100 if total > 0 else 0
    
    improvement = accuracy - 83.9
    
    print(f"\n準確率: {matches}/{total} 個唯一 ID ({accuracy:.1f}%)")
    print(f"改善: +{improvement:.1f}% (從 v13.1 的 83.9% 提升)")
    print()
    
    if differences:
        print(f"\n剩餘差異 (共 {len(differences)} 筆):")
        print("-" * 80)
        
        # 分類剩餘差異
        type_errors = []
        digit_errors = []
        other_errors = []
        
        for diff in differences:
            ours_fields = diff['ours'].split(';')
            official_fields = diff['official'].split(';')
            
            # 檢查是否為 TYPE 錯誤
            if len(ours_fields) > 2 and len(official_fields) > 2:
                if ours_fields[2] != official_fields[2]:
                    type_errors.append(diff)
                    continue
            
            # 檢查是否為純數字差異
            has_digit_diff = False
            for idx in [23, 24, 25, 27, 28]:  # SD, HD, H, PITCH, AZ
                if idx < len(ours_fields) and idx < len(official_fields):
                    if ours_fields[idx] != official_fields[idx]:
                        has_digit_diff = True
                        break
            
            if has_digit_diff:
                digit_errors.append(diff)
            else:
                other_errors.append(diff)
        
        if type_errors:
            print(f"\nTYPE 欄位錯誤 ({len(type_errors)} 筆):")
            for diff in type_errors[:5]:
                ours_fields = diff['ours'].split(';')
                official_fields = diff['official'].split(';')
                print(f"  ID={diff['id']}: '{ours_fields[2]}' vs '{official_fields[2]}'")
        
        if digit_errors:
            print(f"\n數字欄位差異 ({len(digit_errors)} 筆):")
            for diff in digit_errors[:10]:
                ours_fields = diff['ours'].split(';')
                official_fields = diff['official'].split(';')
                print(f"  ID={diff['id']}:")
                for idx in [23, 24, 25, 27, 28]:
                    if idx < len(ours_fields) and idx < len(official_fields):
                        if ours_fields[idx] != official_fields[idx]:
                            print(f"    欄位[{idx}]: '{ours_fields[idx]}' vs '{official_fields[idx]}'")
                            break
        
        if other_errors:
            print(f"\n其他差異 ({len(other_errors)} 筆):")
            for diff in other_errors[:5]:
                print(f"  ID={diff['id']}")
    
    print()
    print("=" * 80)
    
    if accuracy >= 99.5:
        print("\nRESULT: 100% 成功！可以發布！")
    elif accuracy >= 95:
        print(f"\nRESULT: 非常接近 ({accuracy:.1f}%)！")
        print("只剩少數數字異常需要處理")
    elif accuracy >= 90:
        print(f"\nRESULT: 顯著改善 ({accuracy:.1f}%)！")
        print("Context-Aware 過濾器有效，需要加強數字驗證")
    
    print("=" * 80)
    
    return accuracy, differences

if __name__ == "__main__":
    test_field_specific_filter()

