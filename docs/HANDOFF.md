...
**最後修訂**：2026-06-29（Git：內部團隊 GitHub Flow — clone org repo、feature branch + PR、保護 `main`）。  
...
| GitHub 認證 | 首次 push | 跳瀏覽器登入自己帳號；或帳號+PAT | 需先被加為 `icmnlab` org 成員或 repo **Collaborator (Write)** |
...
**帳號與權限（內部團隊 — 與 Google/Meta/Amazon 等 org repo 相同做法）**
- 每位開發者用**自己的 GitHub 帳號**；由 `icmnlab` 加為 **org 成員**或兩 repo 的 **Collaborator (Write)**。
- **`main` 受 branch protection 保護**：不可直接 push `main`；一律 **feature branch → Pull Request → CI 綠燈 → merge**。
- 開發者 push **功能分支**到 org repo（`origin` = `icmnlab`），**不是** push 到 `main`。
- commit 作者為各自 `git config` 設定的身分；merge 後貢獻出現在你的 GitHub profile。
- **Fork** 僅供**沒有 org 寫入權**的外部貢獻者（見 `DEVELOPMENT_WORKFLOW.md` §External contributors）。
...
# 3) clone org repo（接受 collaborator / org 邀請後）
...
git push -u origin feat/ble-export              # 推功能分支到 org repo
# GitHub 開 Pull Request → CI → review → Merge 進 main
git checkout main && git pull origin main      # 合併後本地同步
