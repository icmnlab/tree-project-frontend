...
**最後修訂**：2026-06-29（Git：Fork + PR 至 `icmnlab`，非 Collaborator 直推）。  
**文件入口（英文正本）**：[`docs/README.md`](./README.md) → [`ONBOARDING_READING_PATH.md`](./ONBOARDING_READING_PATH.md)。
...
| GitHub 認證 | 首次 push | 跳瀏覽器登入自己帳號；或帳號+PAT | 推送到**自己的 fork**（`origin`），非 `icmnlab` 直推 |
...
**帳號與權限（Fork + PR，一線團隊標準做法）**
- 每位開發者用**自己的 GitHub 帳號**；在 GitHub 上 **Fork** `icmnlab/tree-project-backend` 與 `tree-project-frontend` 到自己的帳號。
- **不要**把一般開發者加為 org repo 的 Collaborator（Write）— 避免直接 push `icmnlab/main`。
- 日常流程：在本機 `origin`（fork）開分支 → push → 對 **`icmnlab/main` 開 Pull Request** → CI 綠燈 → **org 維護者** merge。
- commit 作者仍為各自 `git config` 設定的身分；merge 後貢獻會出現在你的 GitHub profile。
...
#    第一次 git clone / git push 時，會自動跳出瀏覽器 → 登入自己的 GitHub 帳號 → 完成。
...
# 3) Fork + clone（在 GitHub 網頁先 Fork icmnlab 兩 repo，再 clone 自己的 fork）
git clone https://github.com/<你的GitHub帳號>/tree-project-backend.git
git clone https://github.com/<你的GitHub帳號>/tree-project-frontend.git
cd tree-project-backend && git remote add upstream https://github.com/icmnlab/tree-project-backend.git
cd ../tree-project-frontend && git remote add upstream https://github.com/icmnlab/tree-project-frontend.git

# 4) 之後依本檔 §4「本機跑起來」安裝相依、建 .env / key.properties
```

**若先前已 clone icmnlab 本體（舊交接方式）**：見 `DEVELOPMENT_WORKFLOW.md` §Fork and remotes — 將 `origin` 改指向你的 fork，新增 `upstream` → icmnlab。

**標準流程**
```bash
git checkout main && git pull upstream main      # 從 org 正本同步
git checkout -b feat/ble-export                 # 開功能分支
# ...改程式、commit...
git add -A && git commit -m "feat: BLE 量測匯出 CSV"
git push -u origin feat/ble-export              # 推到自己的 fork
# GitHub：對 icmnlab/<repo> 開 Pull Request → CI → 維護者 Merge 進 main
git checkout main && git pull upstream main      # 合併後本地同步
...
git diff                         # 看未暫存的差異
git pull upstream main             # 從 org 正本同步
