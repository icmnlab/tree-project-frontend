...
| #   | 步驟                                      | 預期結果      | 勾選  |
| --- | --------------------------------------- | --------- | --- |
| A1  | 管理後台 → 邀請碼管理 → 建立（綁專案、需審核） | 回傳可複製邀請碼 | [ ] |
| A2  | 登入頁 → 註冊 → 新帳號（審核碼需管理員啟用） | 待審核無法登入；啟用後可登入 | [ ] |
| A3  | 新帳號僅能看到邀請碼綁定的專案 | 符合 `project_codes` | [ ] |
...
| #   | 步驟 | 預期結果 | 勾選 |
| --- | --- | --- | --- |
| P1  | 登入頁 → 忘記密碼 → 輸入帳號 → 申請 | 成功提示（不洩漏帳號是否存在） | [ ] |
| P2  | 業務管理員 → 待重設密碼 | 列表顯示 6 位碼，可複製 | [ ] |
| P3  | 使用者輸入帳號 + 重設碼 + 新密碼 | 重設成功，可用新密碼登入 | [ ] |


**注意**：後端 `jwtAuth` 須放行 `/register`、`/password-reset-*`（公開端點）；部署後請重啟 Node。
...
- **修正紀錄上傳**預設關閉（`ENABLE_ML_CORRECTION_UPLOAD`）；研究用資料請走管理後台「研究資料蒐集」
- 地圖「全部／全部」：已支援 bbox 拖曳載入；全量仍可能較慢
