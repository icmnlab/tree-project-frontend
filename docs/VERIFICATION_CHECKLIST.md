...
| B8 | 邊界頁 → 右下選單「貼上座標」→ 貼 `(120.12,23.26)` 等 ≥3 點 → 解析 | 預覽顯示頂點數；地圖載入後可微調再儲存 | [ ] |
| B9 | 貼上自相交座標（蝴蝶結順序） | 預覽提示自相交，可選「依角度重排後載入」；強行儲存被後端 400 拒絕 | [ ] |
| B10 | 邊界頁 → 「匯入 KML/GeoJSON」→ 選 Google Earth 匯出的 .kml/.kmz | 預覽顯示「WGS84」、頂點數、面積；載入後可儲存 | [ ] |
| B11 | 匯入 TWD97/TM2 (EPSG:3826) 的 .geojson | 預覽顯示偵測座標系統並已轉 WGS84，位置正確落在台灣 | [ ] |
