# 邊界輸入測試樣本

提供給「專案邊界 → 更多輸入方式」功能做實機測試用的範例檔。
所有樣本是台南附近、同一個四邊形邊界（不同格式/座標系），匯入後應落在同一位置、形狀相同。

| 檔案 | 用途 | 測試項 |
|------|------|--------|
| `coords_sample.txt` | 貼上座標（WGS84，經度,緯度） | B8 |
| `coords_sample_badrow.txt` | 防呆示範：第 3 筆故意掉小數點 | B10（應出現「疑似缺少小數點」並略過該行） |
| `sample_boundary_wgs84.geojson` | 匯入 GeoJSON（WGS84） | B9 |
| `sample_boundary_twd97.geojson` | 匯入 GeoJSON（TWD97/TM2, EPSG:3826）→ 後端自動轉 WGS84 | B9 |
| `sample_boundary.kml` | 匯入 KML（Google Earth 匯出格式） | B9 |
| `coords_complex_pond.txt` | **複雜（凹）多邊形**：環境學院魚塭 9 點，貼上座標 | B12（驗證可畫非四方形/凹形且不自相交） |
| `sample_boundary_complex.geojson` | 同上凹多邊形的 GeoJSON 版本 | B12 |
| `sample_boundary_complex.kml` | 同上凹多邊形的 **KML** 版本（測 KML 匯入凹形 + 與匯出雙向） | B15 |
| `sample_boundary_points_only.kml` | **只有圖釘點、無多邊形**的 KML（模擬使用者只在 Google Earth 點地標） | B20（應依點順序連成邊界並警告） |
| `coords_scrambled_convex.txt` | **亂序凸四邊形**（對角交錯）：示範自相交偵測 + 自動重排 | B13/B16（解析後應警告自相交；按「自動重排」後恢復正常）|

## 座標順序與「自動重排」機制（重要）

邊界是「依點的先後順序連線」而成，所以**順序決定形狀**。系統的容錯分三層：

1. **有序來源（KML / GeoJSON / 手動依序點選 / 從樹木建議）**：順序天生正確，直接使用。
2. **貼上座標 / 手動畫到自相交**：送出前偵測自相交，提供「自動重排」：
   - 先**依角度重排**（對**凸形**最佳，如亂序四方形可完全還原）。
   - 仍自相交再試**最近鄰連線**（對**細長/部分凹形**較佳）。
   - 兩者皆無法消除（複雜凹形）時會明確提示「請手動調整頂點順序」，不會硬存（後端 `turf.kinks` 也會擋下）。
3. `lng,lat` 與 `lat,lng` 會依台灣經緯度範圍自動判斷；無法判斷時用對話框上的「假設順序」。

> **誠實說明極限**：把「一堆無序的點」唯一還原成「使用者心中那個凹多邊形」在幾何上是不唯一的（多邊形化問題），任何演算法都做不到 100%。因此**凹形請用正確順序輸入，或直接匯入 KML/GeoJSON**（檔案保留繞行順序，最穩）。

## Google Earth KML 的多種畫法都吃得下

同一份 Google Earth KML 內可能同時有「圖釘(Point)、路徑(LineString)、多邊形(Polygon)」。匯入時的優先序：

1. **有多邊形** → 直接採用多邊形外環（最準，建議學院用「多邊形」工具畫）。
2. **沒有多邊形、但有路徑** → 把路徑視為封閉邊界（會警告）。
3. **只有圖釘點（≥3）** → 依文件順序連成邊界（會警告，順序不對可在匯入預覽用「自動重排」或回 Google Earth 改畫多邊形）。

> 學院實際給的 `未命名的地圖專案.kml` 同時含 9 個圖釘 + 1 個多邊形「區塊1」，系統會正確採用多邊形（實測 9 頂點、約 12.1 公頃、不自相交）。

## 匯出 KML（與匯入雙向）

已儲存邊界的區，在「區邊界繪製頁」右上角會出現 **匯出（分享）圖示**：
- 點擊即下載該區邊界的 `.kml`，Android 會以 **Google Earth**（若已安裝）或檔案總管開啟。
- 匯出的 KML 可再用 `匯入 KML/GeoJSON` 讀回，座標完全一致（lng,lat,0、環自動閉合）。

## 使用方式

1. 把要測的檔案傳到手機（雲端硬碟／LINE Keep／USB 皆可）。
2. App → 專案邊界繪製頁 → 底部「更多輸入方式」：
   - **貼上座標**：開啟 `coords_sample.txt`，複製內容貼入。
   - **匯入 KML/GeoJSON**：選 `.geojson` 或 `.kml`。
3. 確認預覽地圖上的多邊形位置/形狀正確 → 儲存。
4. 三個檔（wgs84 / twd97 / kml）匯入後邊界應**完全重疊**（驗證座標系自動處理正確）。

## 正確結果（參考）

四個頂點（WGS84，經度, 緯度）：
```
120.1210000, 23.2640000
120.1245000, 23.2638000
120.1250000, 23.2585000
120.1205000, 23.2588000
```

> 這些是「測試資料」，正式上線不會匯入；可自行替換成環境學院的真實座標。
