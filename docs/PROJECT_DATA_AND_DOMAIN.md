...
> 更新：2026-06-08  
> 對照：`DATABASE_NORMALIZATION.md`、`BOUNDARY_SYSTEM_DESIGN.md`
...
- 主要實體均已拆出主檔並以 FK 關聯：`projects`（`project_code` UNIQUE）、
  ...
  `tree_images`（schema 與 2NF 遷移已合併於 `tree_images.pg.sql`）。
...
- **樹木生命週期**（migration 31）：`tree_survey.lifecycle_status`（active|dead|fallen|removed）
  與 `retired_at` / `retired_reason`，相依於代理鍵 `id`、不影響 2NF/3NF。**淘汰木（枯死/倒塌/移除）
  不計入活立木碳匯總計與在庫統計**（政府活立木生物量法），但保留歷史與照片、可復原；
  詳見 `CARBON_CALCULATION.md`、`SURVEY_HISTORY.md`。照片可選綁定某次量測（`tree_images.measurement_id`，migration 32）。
